...
class CONTROL_CONSOLE_REQUEST_APP {
    ...
    function ctrl_auth() {
        ...
        foreach ($_arr_userIds['user_ids'] as $_key=>$_value) {
            ...
            if ($_arr_userRow['rcode'] == 'y010102') {
                $_arr_userResult = $this->mdl_belong->mdl_submit($_value, $_num_appId);
            }
        }

        if ($_arr_userResult['rcode'] == 'y070101' || $_arr_userResult['rcode'] == 'y070103') {
            $_str_rcode = 'y070401';
        }
    ...
    function ctrl_notify() {
        ...
        $_arr_data['sign'] = $this->obj_sign->sign_make(json_encode($_arr_data), fn_baigoCrypt($_arr_appRow['app_key'], $_arr_appRow['app_name']), $_arr_appRow['app_secret']);
        ...
        $_arr_notify = fn_http($_arr_appRow['app_url_notify'] . $_str_conn . 'm=notify', $_arr_data, 'get');
        //print_r($_arr_notify);
