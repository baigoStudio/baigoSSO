...
var _err=0;var thisForm=$(this);var el=this;var _status;var defaults={class_name:{success:"baigoValidator-success",err:"baigoValidator-err",loading:"baigoValidator-loading"},class_attach:{success:"has-success",err:"has-error"}};var opts=$.extend(true,defaults,options);var outPut=function(_msg_selector,_group_selector,_msg,_class,_attach_class){$(_msg_selector).empty();$(_msg_selector).append(_msg);$(_msg_selector).attr("class",_class);$(_msg_selector).show();if(typeof _group_selector!="undefined"&&_group_selector.length>0){$(_group_selector).removeClass(opts.class_attach.success+" "+opts.class_attach.err);$(_group_selector).addClass(_attach_class);}};var getLength=function(_str){var _numLenth=0;if(_str){for(var i=0;i<_str.length;i++){var _chkCode=_str.charCodeAt(i);if(_chkCode<0x007f){_numLenth++;}else if(_chkCode<=0x07ff){_numLenth+=2;}else if(_chkCode<=0xffff){_numLenth+=3;}else{_numLenth+=4;}}}
