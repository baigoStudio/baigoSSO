/*
v2.0.1 jQuery baigoClear plugin 表单 ajax 清理插件
  ...
  $.fn.baigoClear = function(options) {
    ...
    var opts_default = {
      ...
      tpl: '<div class="bg-clear my-3 collapse">' +
        ...
        '</div>' +
        '<div class="bg-msg mt-3 alert"></div>' +
      '</div>'
    };
    ...
    var process = {
      output: function(msg, status_class) {
        ...
        obj_form.find(opts.selector.msg).text(msg);
      },
      submitFunc: function(page, min_id, max_id) {
        ...
        $.ajax({
          ...
          error: function(result){
            process.output(result.status + ' ' + result.statusText, opts.class_msg.err);
          },
          success: function(result) { //读取返回结果
            ...
            switch (result.status) {
              case 'err':
                ...
                process.output(_msg_result, opts.class_msg.err);
                _count  = 0;
                page    = 1;
              break;
              ...
              case 'complete':
                ...
                process.output(_msg_result, opts.class_msg.success);
              ...
              case 'next':
                ...
                process.output(_msg_result, opts.class_msg.submitting);
                _progress_width = 20;
