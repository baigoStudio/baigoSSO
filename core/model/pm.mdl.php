...
class MODEL_PM {
    ...
    function input_send_api($arr_inputSend) {
        $_str_pmTitle   = '';
        $_str_pmContent = '';
        $_str_pmTo      = '';

        if (isset($arr_inputSend['pm_title'])) {
            $_str_pmTitle = $arr_inputSend['pm_title'];
        }

        if (isset($arr_inputSend['pm_content'])) {
            $_str_pmContent = $arr_inputSend['pm_content'];
        }

        if (isset($arr_inputSend['pm_to'])) {
            $_str_pmTo = $arr_inputSend['pm_to'];
        }

        $_arr_pmTitle = fn_validate($arr_inputSend['pm_title'], 0, 90);
        switch ($_arr_pmTitle['status']) {
            case 'too_long':
                return array(
                    'rcode' => 'x110202',
        ...
        $_arr_pmContent = fn_validate($arr_inputSend['pm_content'], 1, 900);
        switch ($_arr_pmContent['status']) {
            case 'too_short':
                return array(
                    'rcode' => 'x110201',
        ...
        $_arr_pmTo = fn_validate($arr_inputSend['pm_to'], 1, 0);
        switch ($_arr_pmTo['status']) {
            case 'too_short':
                return array(
                    'rcode' => 'x110205',
    ...
    function input_ids_api($arr_inputIds) {
        ...
        if (isset($arr_inputIds['pm_ids'])) {
            $_str_pmIds = fn_getSafe($arr_inputIds['pm_ids'], 'txt', '');
        }
