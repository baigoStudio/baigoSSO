...
return "<h3>所有应用</h3>
    ...
    <h3>查看应用</h3>
    <p>
        应用列表的查看菜单，进入如下界面。在此，您可以获取调用 API 接口所需要的信息。如果 APP KEY 泄露，可以在此点击重置 APP KEY 按钮来进行更换，原 APP KEY 将作废。
    </p>
    ...
    <h3>授权用户</h3>
    <p>
        点击应用列表的授权用户菜单，进入如下界面。在此，您可以对应用进行授权用户的操作，授权用户是指应用拥有这些用户的授权，可以编辑、删除取得授权的用户，此功能有助于提高用户数据库的安全性。如果应用权限中允许操作所有用户，则不受限制。
    </p>
