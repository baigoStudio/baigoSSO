...
class MODEL_USER_FORGOT extends MODEL_USER {
    ...
    function input_byqa($arr_inputForgot) {
        $_arr_userInput = $this->input_user($arr_inputForgot);
        ...
        for ($_iii = 1; $_iii <= 3; $_iii++) {
            $_arr_userAnsw[$_iii] = '';
            if (isset($arr_inputForgot['user_sec_answ_' . $_iii])) {
                $_arr_userAnsw[$_iii] = $arr_inputForgot['user_sec_answ_' . $_iii];
            }
        ...
        if (isset($arr_inputForgot['user_pass_new'])) {
            $_str_passNew = $arr_inputForgot['user_pass_new'];
        }

        if (isset($arr_inputForgot['user_pass_confirm'])) {
            $_str_passConfirm = $arr_inputForgot['user_pass_confirm'];
        }
        ...
        $_arr_userPassConfirm = fn_validate($_str_passConfirm, 1, 0);
        switch ($_arr_userPassConfirm['status']) {
            case 'too_short':
                return array(
                    'rcode' => 'x010224',
