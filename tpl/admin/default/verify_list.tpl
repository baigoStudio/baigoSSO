...
{$cfg = [
    ...
    baigoSubmit    => "true",
    tokenReload    => "true",
    str_url        => "{$smarty.const.BG_URL_ADMIN}ctl.php?mod=verify&{$tplData.query}"
]}
...
{include "{$smarty.const.BG_PATH_TPL}admin/default/include/admin_head.tpl" cfg=$cfg}
    ...
    <div class="form-group">
        <ul class="nav nav-pills nav_baigo">
            <li>
                <a href="{$smarty.const.BG_URL_HELP}ctl.php?mod=admin&act_get=verify" target="_blank">
        ...
        <div class="panel panel-default">
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        {foreach $tplData.verifyRows as $value}
                            {if $value.verify_status == "enable"}
                                {$_css_status = "success"}
                                {$_str_status = $status.verify[$value.verify_status]}
                            ...
                            {/if}
                            <tr>
                                <td class="text-nowrap td_mn"><input type="checkbox" name="verify_id[]" value="{$value.verify_id}" id="verify_id_{$value.verify_id}" group="verify_id" class="validate chk_all"></td>
...
{include "{$smarty.const.BG_PATH_TPL}admin/default/include/admin_foot.tpl" cfg=$cfg}
    ...
    var opts_validator_list = {
        verify_id: {
            ...
            msg: { id: "msg_verify_id", too_few: "{$alert.x030202}" }
        },
        act_post: {
            length: { min: 1, max: 0 },
