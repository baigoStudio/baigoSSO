...
2 用法
    ...
    2.2 初始化 baigoValidator，如：
        obj_form = $("#cate_form").baigoValidator(opts_validate, options);
        opts_validate 参数为配置对象，类型为 JSON，名称可以自定义
        options 为附加参数
    ...
    2.5 触发验证，如：
        var obj_form = $("#cate_form").baigoValidator(opts_validate, options);
        $("#cate_form").submit(function(){
            obj_form.verify();
        });

    2.6 附加参数，如：
        class_name: { //css 类，此 css 类名将附加至提示信息的元素上，默认为 bootstrap 关于表单验证的 css 类
            success: "text-success", //验证通过
            err: "text-danger",      //验证错误
            loading: "text-info"     //正在验证
        },
        class_attach: { //附加 css 类，此 css 类名将附加至表单组的元素上，默认为 bootstrap 4.0 关于表单验证的 css 类
            success: "is-valid", //验证通过
            err: "is-invalid"    //验证错误
        },
        msg_global: { //总体消息，主要用于显示错误
            selector: { //选择器
                parent: ".bg-validator-box", //父选择器
                content: ".bg-content",   //内容
                icon: ".bg-icon",         //图标
                msg: ".bg-msg",           //消息
            },
            msg: "Error",                    //错误消息
            class_err: "alert-danger",       //错误 css 类
            class_icon: "oi oi-circle-x", //错误图标
            tpl: "<div class=\"alert alert-dismissible bg-content\"><button type=\"button\" class=\"close\" data-dismiss=\"alert\">&times;</button><span class=\"bg-icon\"></span>&nbsp;<span class=\"bg-msg\"></span></div>", //消息模板
            delay: 5000 //消失时间
        }
